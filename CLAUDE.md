...
**Deep-link sync (Android)** — `interest://sync-anki` is handled by `SyncActivity` (transparent, `excludeFromRecents`, `noHistory`, separate `taskAffinity`, `singleInstance`), which runs the headless `ankiSyncMain` Dart entrypoint (`lib/anki_sync_entrypoint.dart`) on its own Flutter engine and posts progress via `SyncNotifier`, **without foregrounding `MainActivity`**. `MainActivity` is a plain `FlutterActivity`. The `AnkiBridge` (AnkiDroid ContentProvider) and the `com.nimeesh.interest/sync` notification channel are registered on `SyncActivity`. This native path compiles and the APK builds but is **not yet device-verified**; a zero-flash variant would use a foreground Service + background isolate. Do NOT route the deep link back through the main UI.
